...
function getDefaultChunkSize(outputFormat) {
    switch (outputFormat) {
        case 'rag':
            return 500;
...
// LLM Enrichment
async function enrichChunks(chunks, config, provider, apiKey) {
    ...
    for (let i = 0; i < chunks.length; i += batchSize) {
        ...
        const enrichedBatch = await Promise.all(batch.map(chunk => enrichSingleChunk(chunk, config, provider, apiKey)));
...
async function generateEmbeddings(chunks, model, apiKey) {
...
function formatOutput(chunks, format, enrichment) {
    switch (format) {
        case 'rag':
...
function generateMarkdownDocument(chunks, enrichment) {
    const lines = [];
...
function generateOpenAIFormat(chunks, enrichment) {
    const records = [];
    for (const chunk of chunks) {
...
function generateAlpacaFormat(chunks, enrichment) {
    const records = [];
    for (const chunk of chunks) {
        if (enrichment.enabled && chunk.enrichment?.questions) {
            for (const question of chunk.enrichment.questions) {
...
Actor.main(async () => {
    ...
    if (enrichmentConfig.enabled && config.llmApiKey) {
        enrichedChunks = await enrichChunks(allChunks, enrichmentConfig, config.llmProvider, config.llmApiKey);
    }
    // Generate embeddings if enabled
    let finalChunks = enrichedChunks;
    if (config.generateEmbeddings && config.embeddingApiKey) {
        finalChunks = await generateEmbeddings(enrichedChunks, config.embeddingModel, config.embeddingApiKey);
    }
    // Format output
