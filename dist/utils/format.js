 ...
 */
export function generateMarkdownDocument(chunks, enrichment) {
    const lines = [];
 ...
 */
export function generateOpenAIFormat(chunks, enrichment) {
    const records = [];
    for (const chunk of chunks) {
 ...
 */
export function generateAlpacaFormat(chunks, enrichment) {
    const records = [];
    for (const chunk of chunks) {
        if (enrichment.enabled && chunk.enrichment?.questions) {
            for (const question of chunk.enrichment.questions) {
