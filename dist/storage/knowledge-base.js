 ...
 */
export function generateKBId(sourceUrl) {
 ...
 */
async function addToKBIndex(id) {
    const store = await Actor.openKeyValueStore();
    const index = await store.getValue(KB_INDEX_KEY) ?? [];
 ...
 */
async function removeFromKBIndex(id) {
    const store = await Actor.openKeyValueStore();
    const index = await store.getValue(KB_INDEX_KEY) ?? [];
 ...
 */
export async function createKnowledgeBase(sourceUrl, config) {
...
export async function getKnowledgeBase(id) {
    const store = await Actor.openKeyValueStore();
    return store.getValue(`${KB_META_PREFIX}${id}${KB_META_SUFFIX}`);
}
/**
 ...
 */
export async function updateKnowledgeBaseStatus(id, status) {
    const kb = await getKnowledgeBase(id);
    if (!kb) {
 ...
 */
export async function updateKnowledgeBaseStats(id, stats) {
    const kb = await getKnowledgeBase(id);
    if (!kb) {
        throw new Error(`Knowledge base not found: ${id}`);
    }
 ...
 */
export function convertToKBChunks(chunks, knowledgeBaseId) {
 ...
 */
export async function saveChunks(kbId, chunks) {
    if (chunks.length === 0) {
        return;
    }
 ...
 */
export async function getChunks(kbId, options = {}) {
 ...
 */
export async function searchKnowledgeBase(kbId, query, limit = 10) {
 ...
 */
export async function semanticSearch(kbId, queryEmbedding, limit = 10) {
    const scored = [];
