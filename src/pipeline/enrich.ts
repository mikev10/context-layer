...
import type { Chunk, EnrichedChunk, EnrichOptions, EnrichmentConfig } from '../types/index.js';
...
function parseEnrichmentResponse(
    ...
    try {
        ...
        if (config.generateQA && parsed.questions) {
            enrichment.questions = parsed.questions.map(
...
export async function enrichChunks(
    ...
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        ...
        const enrichedBatch = await Promise.all(
            batch.map(chunk =>
                enrichSingleChunk(chunk, config, options.llmProvider, options.llmApiKey)
            )
        );
