 ...
 */
export async function createJob(runId: string, knowledgeBaseId: string): Promise<Job> {
    const job: Job = {
        id: runId,
        knowledgeBaseId,
        status: 'pending',
...
export async function getJob(runId: string): Promise<Job | null> {
    const store = await Actor.openKeyValueStore();
    return store.getValue<Job>(`${JOB_PREFIX}${runId}`);
}
 ...
 * @param runId - Actor run ID
 */
export async function completeJob(runId: string): Promise<void> {
    const job = await getJob(runId);
    if (!job) {
        log.warning(`Job not found: ${runId}`);
        return;
